...
docker run -d --restart always --name $FORWARDER --publish $GWPORT:$GWPORT/udp -v /etc/thingsix-forwarder:/etc/thingsix-forwarder $DOCKER_PULL_URL$version_git --config /etc/thingsix-forwarder/$MY_CONFIG_FILE
